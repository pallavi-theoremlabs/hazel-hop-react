...
import { lookupBankByFdic, submitInterest } from '../../services/public/api'
...
function openCase(navigate, result) {
  const params = new URLSearchParams()
  if (result.institution_id) params.set('institution_id', result.institution_id)
  const query = params.toString()
  navigate(`${result.next_path}${query ? `?${query}` : ''}`)
}
  ...
  if (result) {
    return <div className="auth public-interest"><main className="auth-card" id="main">
      ...
      {result.eligible && result.next_path && <section className="local-dev-simulation" aria-label={t('public:submitInterest.integrationLabel')}>
        ...
        <div className="actions"><Button onClick={() => openCase(navigate, result)}>{t('public:submitInterest.continueOnboarding')}</Button></div>
      </section>}
    </main></div>
  }
